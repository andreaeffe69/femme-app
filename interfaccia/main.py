...
""",
    ...
    def crea_splash(self):
        """Splash screen con solo video a schermo intero"""
        from kivy.uix.boxlayout import BoxLayout
        ...
        layout.size_hint = (1, 1)  # Occupa tutto lo spazio disponibile
        
        # VIDEO a schermo intero
        video_path = os.path.join(os.path.dirname(__file__), 'assets', 'splash_femme.mp4')
        
        if os.path.exists(video_path):
            self.video = Video(source=video_path, state='play')
            self.video.size_hint = (1, 1)  # Occupa tutto lo schermo
            layout.add_widget(self.video)
            print("✅ Video splash avviato")
        else:
            # Fallback se il video non c'è
            from kivy.uix.label import Label
            layout.add_widget(Label(text="FEMME", font_size='80sp'))
            Clock.schedule_once(self.carica_app, 2.0)
            return layout
        
        # Dopo 3 secondi carica l'app
        Clock.schedule_once(self.carica_app, 3.0)
